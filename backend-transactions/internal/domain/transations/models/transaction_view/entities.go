...
func (r AccountRaw) Entity() (AccountEntity, error) {
	var err error
	...
	entity.ID, err = vo.UUIDFromRaw(r.ID)
	if err != nil {
		return AccountEntity{}, errors.Wrap(err).WithCode("ID")
	}

	entity.Name, err = vo.StringFromRaw(r.Name)
	if err != nil {
		return AccountEntity{}, errors.Wrap(err).WithCode("NAME")
	}
...
func (e AccountEntity) Raw() AccountRaw {
	return AccountRaw{
		ID:   e.ID.Raw(),
		Name: e.Name.Raw(),
	}
